...
sys.argv.append('--rec_model_dir=./inference/en_PP-OCRv3_rec/')
sys.argv.append('--rec_char_dict_path=/home/aistudio/data/data120624/captcha/word_dict.txt')
infer_model = init_infer_model(utility.parse_args())

app = FastAPI()


@app.post("/captcha")
async def parse_captcha(file: UploadFile):
    file_bytes = file.file.read()
    img = cv2.imdecode(np.frombuffer(file_bytes, np.uint8), cv2.IMREAD_COLOR)
    rec_res, _ = infer_model([img])
    return {"filename": file.filename, "predict": rec_res[0][0], "confidence": rec_res[0][1]}
