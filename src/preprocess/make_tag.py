...
@click.option('--origin-data-dir', type=str, default='splitted')
@click.option('--target-data-dir', type=str, default='train')
def run(mode, data_dir, origin_data_dir, target_data_dir):
    if mode == 'auto-tag':
        ...
        origin_dir = root_dir / 'dataset' / origin_data_dir
        target_dir = root_dir / 'dataset' / target_data_dir
        manual_tag(origin_dir, target_dir)
    elif mode == 'gen-label':
        if data_dir is None:
