...
class CaptchaDataset(Dataset):
    ...
    channel: 生成的标签需要的颜色类型，可选两种模式，第一种是指定具体颜色类型，第二种是包含所有类型
    """
    ...
    def __init__(
        ...
        self.transform = transforms.Compose([transforms.Normalize(mean=self.mean, std=self.std), transforms.ToTensor()])
    ...
    def __getitem__(self, idx):
        if self.auto_gen:  # 自动生成
            ...
            if self.channel not in ["text", "all"]:
        ...
        else:  # 读取所有颜色通道标签，拼接为一维数组
            label_arr = []
            for channel in self.channels:
                if channel == "all":
                    continue
                label_seq = [self._vocabulary_dict[t] for t in label_map.get(channel, [])]
                if len(label_seq) < self.max_len:
                    label_seq += [-1] * (self.max_len - len(label_seq))
                label_arr += label_seq
            label_arr = paddle.to_tensor(label_arr, dtype="int32")
