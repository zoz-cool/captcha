...
class Trainer:
    ...
    def _init_data(self):
        ...
        self.test_dataset = dataset.CaptchaDataset(
            ...
            dataset_dir=self.args.dataset_dir,
            auto_gen=self.args.auto_num > 0 and self.args.dataset_dir is None,
            auto_num=self.args.auto_num // 2,  # 自动生成时测试集数量为训练集的一半
            mode="test",
            color=self.args.channel,
        )
        self.test_loader = DataLoader(
            dataset=self.test_dataset, batch_size=self.args.batch_size
        )
    ...
    def _init_model(self):
        ...
        self.scheduler = paddle.optimizer.lr.ReduceOnPlateau(
            learning_rate=self.args.lr, factor=0.1, patience=10
        )
        ...
        if self.args.pretrained:
            ...
            self.model.set_state_dict(
                paddle.load(os.path.join(self.args.pretrained, "model.pdparams"))
            )
            self.optimizer.set_state_dict(
                paddle.load(os.path.join(self.args.pretrained, "optimizer.pdopt"))
            )
    ...
    def train_step(self, epoch_id, batch_id, input_data):
        ...
        input_lengths = paddle.full(
            shape=[out.shape[1]], fill_value=out.shape[0], dtype="int64"
        )
        ...
        if batch_id % 100 == 0:
            print(
                "[%s] Train epoch %d, batch %d, lr %f, loss: %f"
                % (
                    datetime.now(),
                    epoch_id,
                    batch_id,
                    self.scheduler.last_lr,
                    float(loss),
                )
            )
    ...
    def test_epoch(self, epoch_id):
        if (
            epoch_id % self.args.eval_per_epoch == 0 and epoch_id != 0
        ) or epoch_id == self.args.num_epoch - 1:
            # 执行评估
            cer = self.evaluate()
    ...
    def save_epoch(self, epoch_id):
        """保存模型"""
        if (
            (epoch_id % self.args.save_per_epoch == 0 and epoch_id != 0)
            or (epoch_id == self.args.num_epoch - 1)
            or (epoch_id == 0 and self.args.save_per_epoch == 1)
        ):
            os.makedirs(self.args.save_path, exist_ok=True)
            # 移除超出限制的模型
            epoch_dirs = [
                e_dir
                for e_dir in os.listdir(self.args.save_path)
                if re.match(r"^e\d+$", e_dir)
            ]
    ...
    def evaluate(self):
        ...
        for batch_id, (inputs, labels) in enumerate(self.test_loader()):
            ...
            for label in labels:
                label_text = decoder.label_to_string(
                    label, self.test_dataset.vocabulary
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--vocabulary_path", type=str, default=str(proj_dir / "assets/vocabulary.txt")
    )
    parser.add_argument(
        "--save_path", type=str, default=str(proj_dir / "output/checkpoint")
    )
